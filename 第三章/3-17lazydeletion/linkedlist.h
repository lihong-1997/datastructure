...
namespace linkedlist
{
	...
	public:
		...
		~List<T>();
	public:
		bool empty() const { return length == 0; }           //判断链表是否为空
		...
		bool IsLast(Node<T>* positon) const;                 //判断是否为尾结点
		void Insert(T value, Node<T>* positon);              //指定位置后插入元素
		void Insert(T value);                                //链表尾部插入元素
		Node<T>* Find(T value) const;                        //迭代查找指定元素
		Node<T>* Find_recursive(T value) const;              //递归查找指定元素
		Node<T>* Find_Kth(size_t K) const;                   //查找第k个元素
		Node<T>* Find_pre(T value) const;                    //查找元素前一个节点
		void Remove(T value);                                //删除指定元素
		void LazyDelete(T value);                            //懒惰删除
	...
	inline void List<T>::LazyDelete(T value)
	{
		...
		p->deleted = true;
		++delete_count;
		//当被删除元素累计到与非被删除元素一样多时，执行删除操作，删除所有被标记节点
